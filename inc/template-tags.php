...
if (!function_exists('bootstrapBasicCategoriesList')) {
    ...
    function bootstrapBasicCategoriesList($categories_list = '')
    {
        ...
        if ($edit_post_link !== null) {
            return sprintf('<span class="categories-icon glyphicon glyphicon-th-list" title="' . __('Posted in', 'blue-lite') . '"></span> %1$s', $categories_list);
        } else {
            return sprintf('<span class="categories-icon glyphicon glyphicon-th-list" style="margin-left:5px;" title="' . __('Posted in', 'blue-lite') . '"></span> %1$s', $categories_list);
        }
    }
}
...
if (!function_exists('bootstrapBasicComment')) {
    ...
    function bootstrapBasicComment($comment, $args, $depth)
    {
        ...
        if ('pingback' === $comment->comment_type || 'trackback' === $comment->comment_type) {
            ...
            esc_html_e('Pingback:', 'blue-lite');
            comment_author_link();
            edit_comment_link(__('Edit', 'blue-lite'), '<span class="edit-link">', '</span>');
            echo '</div>';
            echo '</div>';
        } else {
            echo '<li id="comment-';
            ...
            printf(_x('%1$s at %2$s', '1: date, 2: time', 'blue-lite'), get_comment_date(), get_comment_time()); // phpcs:ignore
            echo '</time>';
            echo '</a>';

            echo ' ';

            edit_comment_link('<span class="fa fa-pencil-square-o "></span>' . __('Edit', 'blue-lite'), '<span class="edit-link">', '</span>');
            ...
            if ('0' == $comment->comment_approved) { // phpcs:ignore Universal.Operators.StrictComparisons.LooseEqual
                ...
                esc_html_e('Your comment is awaiting moderation.', 'blue-lite');
                echo '</div>';
            } //endif;

            // comment author says
            /* translators: %s: Comment author link. */
            printf(__('%s <span class="says">says:</span>', 'blue-lite'), sprintf('<cite class="fn">%s</cite>', get_comment_author_link())); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
            ...
            comment_reply_link(array_merge($args, array(
                ...
                'reply_text' => '<span class="fa fa-reply"></span> ' . __('Reply', 'blue-lite'),
                'login_text' => '<span class="fa fa-reply"></span> ' . __('Log in to Reply', 'blue-lite'),
            )));
            // end reply link
...
if (!function_exists('bootstrapBasicEditPostLink')) {
    ...
    {
        $edit_post_link = get_edit_post_link();
        if ($edit_post_link != null) { // phpcs:ignore Universal.Operators.StrictComparisons.LooseNotEqual
            $edit_btn = '<a class="post-edit-link btn btn-default btn-xs" href="' . $edit_post_link . '" title="' . __('Edit', 'blue-lite') . '"><i class="edit-post-icon glyphicon glyphicon-pencil" title="' . __('Edit', 'blue-lite') . '"></i></a>';
...
if (!function_exists('bootstrapBasicMoreLinkText')) {
    ...
    {
        return __('Continue reading <span class="meta-nav">&rarr;</span>', 'blue-lite');
...
if (!function_exists('bootstrapBasicPagination')) {
    ...
    function bootstrapBasicPagination($pagination_align_class = 'pagination-center pagination-row')
    {
        ...
        if (is_array($pagination_array) && !empty($pagination_array)) {
            echo '<nav class="' . $pagination_align_class . '">'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
            echo '<ul class="pagination">';
            foreach ($pagination_array as $page) {
                if (!is_scalar($page)) {
                    echo '<!-- $page is not scalar type: ' . var_export($page, true) . ' -->' . PHP_EOL; // phpcs:ignore
                    continue;
                }
                ...
                echo '>';
                if (strpos($page, '<a') === false && strpos($page, '&hellip;') === false) {
                    echo '<span>' . $page . '</span>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
                } else {
...
if (!function_exists('bootstrapBasicPostOn')) {
    ...
    function bootstrapBasicPostOn()
    {
        ...
        printf(
            __('<span class="posted-on">Posted on %1$s</span><span class="byline"> by %2$s</span>', 'blue-lite'), // phpcs:ignore
            sprintf(
            ...
            sprintf(
                ...
                esc_attr(sprintf(__('View all posts by %s', 'blue-lite'), get_the_author())),
...
if (!function_exists('bootstrapBasicTagsList')) {
    ...
    {
        return sprintf('<span class="tags-icon glyphicon glyphicon-tags" title="' . __('Tagged', 'blue-lite') . '"></span>&nbsp; %1$s', $tags_list);
